...
```{r}
df_f=read_csv(here("data_raw","FastFoodRestaurants.csv"))
```

Before comparing, we will (for each state where we want to compare) use a random sample from McDonalds located in that state such that we *match* the sample size with the number of Denny's in that state.


Let's try for North Carolina:
...
df_f_nc=df_f %>% filter(province=="NC",name=="McDonald's") %>% slice_sample(n=28)
```

Merge datasets and compute distance:

```{r}
dff_lq_nc <- full_join(df_f_nc, lq_nc, by = character())
